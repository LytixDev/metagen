...
static void write_base(void)
{
    char *base = "/*\n"
                 ...
                 " * 2025 Nicolai Brand (https://lytix.dev)\n"
                 " */\n"
                 "#include <stdbool.h>\n"
                 "#include <stdint.h>\n"
                 "#include <stdio.h>\n"
...
static Str8 type_info_and_identifier(Compiler *compiler, TypeInfo *t, Str8 *identifier)
{
    ...
    if (identifier != NULL) {
        str_builder_append_u8(&sb, ' ');
        str_builder_append_str8(&sb, *identifier);
    }
...
static void gen_global(Compiler *compiler, Symbol *sym)
{
...
static void gen_func(Compiler *compiler, Symbol *sym)
{
    ...
    for (u32 i = 0; i < t->n_params; i++) {
        Str8 param_type = type_info_and_identifier(compiler, t->param_types[i], &t->param_names[i]);
        fprintf(f, "%s", param_type.str);
        if (i != t->n_params - 1) {
            fprintf(f, ", ");
        }
    }
...
void transpile_to_c(Compiler *compiler)
{
    ...
    for (u32 i = 0; i < symt_root->sym_len; i++) {
        Symbol *sym = symt_root->symbols[i];
        if (sym->kind == SYMBOL_GLOBAL_VAR) {
            gen_global(compiler, sym);
        }
    }
