...
TokenKind operator_precedence[TOKEN_TYPE_ENUM_COUNT] = {0}; 
...
static Token next_token(Parser *parser)
{
    ...
    //}
    Token *t = arraylist_get(&parser->tokens, parser->token_pos);
    assert(t != NULL);
    parser->token_pos++;
...
static void init_operator_precedence(void)
{
    operator_precedence[TOKEN_ASSIGNMENT] = 1;
    operator_precedence[TOKEN_EQ] = 3;
    operator_precedence[TOKEN_NEQ] = 3;
    operator_precedence[TOKEN_LESS] = 3;
    operator_precedence[TOKEN_GREATER] = 3;
    operator_precedence[TOKEN_LSHIFT] = 4;
    operator_precedence[TOKEN_RSHIFT] = 4;
    operator_precedence[TOKEN_PLUS] = 5;
    operator_precedence[TOKEN_MINUS] = 5;
    operator_precedence[TOKEN_STAR] = 10;
    operator_precedence[TOKEN_SLASH] = 10;
...
static AstRoot *parse_root(Parser *parser)
{
    AstRoot *root = (AstRoot *)alloc_ast_node(parser->arena, AST_ROOT);
    arraylist_init(&root->global_variables, sizeof(AstNode *));
    arraylist_init(&root->global_functions, sizeof(AstNode *));
    arraylist_init(&root->global_structs, sizeof(AstNode *));
...
AstRoot *parse(Arena *arena, ArrayList tokens, ErrorHandler *e)
{
    ...
    Parser parser = { .arena = arena, .e = e, .tokens = tokens, .token_pos = 0 };
    return parse_root(&parser);
}
